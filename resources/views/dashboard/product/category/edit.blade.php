            ...
            <div class="">
                <div class="bg-primary p-6 xl:px-8 border-2 border-gray-300 rounded-lg">
                    <div class="flex justify-between items-center mb-4 xl:mb-8">
                        <h1 class="font-semibold text-lg">Edit Produk</h1>
                    </div>
                    <div class="jantuk">
                        <form action="/dashboard/product/category/{{ $category->slug_category }}" method="post" class="">
                            @method('put')
                            @csrf
                            <div class="w-full flex flex-wrap justify-between">
                                <div class="w-[48.5%] mb-5">
                                    <label for="name_category" class="font-semibold text-base mb-4 block">Nama Kategori</label>
                                    <input type="text" name="name_category" id="name_category"  value="{{ old('name_category', $category->name_category) }}" class="text-sm p-3 rounded-md w-full border-[1.5px] font-medium border-black border-opacity-[16%]" placeholder="" required />
                                    @error('name_category')
                                ...
                                <div class="w-[48.5%] mb-5">
                                    ...
                                    <input type="text" name="slug_category" id="slug_category"   value="{{ old('slug_category', $category->slug_category) }}" class="text-sm p-3 rounded-md w-full border-[1.5px] font-medium border-black border-opacity-[16%]" required readonly/>
                                    @error('slug_category')
                                        <div class="text-red-500 text-sm">
                                        {{ $message }}
                                        </div>
                            ...
                            <button type="submit" name="create" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Buat Kategori</button>
