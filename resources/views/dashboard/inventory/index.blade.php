...
<body class="font-inter">
    ...
    <main class="w-full md:w-[calc(100%-256px)] md:ml-64 bg-gray-100 min-h-screen transition-all main">
        ...
        <div class="p-6">
            <a href="/dashboard/inventory/create" class="p-3 bg-sky-500 text-white rounded-md text-sm">Buat produk Baru</a>
            <div class="container-box mt-8">
                <table class="table-auto w-full">
                    <thead>
                        <tr>
                            <th class="px-4 py-2 bg-gray-200">Id</th>
                            <th class="px-4 py-2 bg-gray-200">Nama Produk</th>
                            <th class="px-4 py-2 bg-gray-200">Nama Supplier</th>
                            <th class="px-4 py-2 bg-gray-200">Tanggal Order</th>
                    ...
                    <tbody>
                        ...
                        <tr class="bg-gray-100 hover:bg-gray-200">
                            ...
                            <td class="border px-4 py-2">{{  $inventory->product->name_product }}</td>
                            <td class="border px-4 py-2">{{  $inventory->supplier->name }}</td>
                            <td class="border px-4 py-2">{{  $inventory->order_date }}</td>
                            @if($inventory->operation == 'plus')
                            <td class="border px-4 py-2">{{  $inventory->stock }}</td>
                            @else
                            <td class="border px-4 py-2">-{{  $inventory->stock }}</td>
                            @endif
                           
                            <td class="border px-4 py-2">
                                <a href="/dashboard/inventory/{{ $inventory->id }}/edit" class="text-blue-500">Ubah</a>
                            </td>
                            <td class="border px-4 py-2">
                                <form action="/dashboard/inventory/{{ $inventory->id }}" method="post">
                                    @method('delete')
                                    @csrf
