            ...
            <div class="">
                <div class="bg-primary p-4 xl:px-6 border-2 border-gray-300 rounded-lg">
                    <div class="flex justify-between mt-4 xl:mt-6 items-center mb-1 xl:mb-2">
                        <h1 class="font-semibold text-lg">Data Supplier</h1>
                        <a href="/dashboard/supplier/create" class="font-medium text-sm tracking-wide bg-secondary rounded-sm text-white px-4 py-2.5">Buat Supplier Baru</a>
                    </div>
                    <div class="jantuk">
                        <table id="example" data-ordering="false" class="table-auto w-full mx-6 md:mx-auto justify-center text-left mt-6">
                            <thead class="border-b-2 border-gray-300">
                                <tr class="justify-between">
                                    ...
                                    <th class="font-semibold pr-6 py-3 text-[16px]">Alamat</th>
                            ...
                            <tbody>
                                ...
                                <tr class="justify-between text-left">
                                    ...
                                    <td class="pr-6 py-3 text-[16px]">{{  $supplier->name }}</td>
                                    <td class="pr-6 py-3 text-[16px]"><p class="text-left">{{  $supplier->address }}</p></td>
                                    <td class="pr-6 py-3 text-[16px]"><p class="text-left">{{ $supplier->phone }}</p></td>
                                    <td class="py-3 text-[16px] flex gap-2 w-10 md:w-auto items-start">
                                        <a href="/dashboard/supplier/{{ $supplier->id }}/edit" class="text-blue-500">
                                            <img src="/img/kotak lihat.svg" alt="">
                                        </a>
                                        <form id="delete-form-{{ $supplier->id }}" action="/dashboard/supplier/{{ $supplier->id }}" method="post">
                                            @method('delete')
                                            @csrf
                                            <button type="button" onclick="confirmDelete('{{ $supplier->id }}')" class="w-full">
    ...
    <x-slot:alerts>
        @if (session('createSupplier'))
            <script>
                Swal.fire({
                    title: 'Success!',
                    text: '{{ session('createSupplier') }}',
                    icon: 'success',
                    confirmButtonText: 'Kembali'
                });
            </script>
        @elseif (session('updateSupplier'))
            <script>
                Swal.fire({
                    title: 'Success!',
                    text: '{{ session('updateSupplier') }}',
                    icon: 'success',
                    confirmButtonText: 'Kembali'
                });
            </script>
        @elseif (session('deleteSupplier'))
            <script>
                Swal.fire({
                    title: 'Success!',
                    text: '{{ session('deleteSupplier') }}',
                    icon: 'success',
                    confirmButtonText: 'Kembali'
                });
            </script>
